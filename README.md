...
| **Run MCPHub when I sign in** | Starts MCPHub with your Windows or Linux session. Registered against your user account only — no administrator rights, no effect on other accounts — and applied immediately. It lives in the OS rather than in `settings.json`, so turning it off in Task Manager's Startup tab is reflected here, and an exported settings archive never carries it to another machine. |
| **System tray** | Whether minimising and closing hide to the tray instead of exiting. The tray icon's right-click menu also offers **Restart MCPHub**, which stops every managed server cleanly and starts the app again. |
...
**MCPHub won't open and no window appears.** Only one MCPHub may run at a time: it binds a fixed proxy port, starts each service on its own fixed port, and writes to a shared servers folder, so a second instance would fight the first for all three. Starting it again therefore does not start a second copy — it brings the running one's window forward, the same as clicking its tray icon, and exits with code `0`. If the window still does not appear, the running instance did not answer: that launch exits with code `2` and explains itself on standard error, which you will see if you started it from a terminal. Look for the tray icon in the notification area, since **Close to tray** keeps MCPHub alive after you close the window.
